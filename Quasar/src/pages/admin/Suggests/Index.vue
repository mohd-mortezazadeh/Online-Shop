  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="suggests_list" icon="mdi-format-list-text" label="پیشنهاد محصول ها"/>
        </q-tabs>
        <q-separator/>

        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="suggests_list">
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      <q-btn @click="EditSuggest(props.row.id)" style="color: #4facfe" rounded
                      ...
                      |
                      <q-btn
                        @click="DeleteSuggest(props.row.product.title + '-' + props.row.user.username , props.row.id)"
...
export default {
  ...
  methods: {
    ...
    DeleteSuggest (name, id) {
      const app = this
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید پیشنهاد محصول ' + ' ( ' + name + ' ) ' + 'حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.delete(process.env.api + 'api/admin/products/suggests/' + id)
          .then(function () {
            app.showNotif('پیشنهاد محصول مورد نظر با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
            app.GetAllSuggests()
            app.GetRequiredList()
          })
          .catch(function () {
    ...
    EditSuggest (id) {
      ...
      this.$q.loading.show()
      this.$axios.get(process.env.api + 'api/admin/products/suggests/' + id + '/')
        .then(function (response) {
    ...
    GetAllSuggests () {
      ...
      this.$axios.get(process.env.api + 'api/admin/products/suggests/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
