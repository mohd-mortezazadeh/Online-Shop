  ...
  <q-dialog v-model="_dialog">
    <q-card style="width: 750px; max-width: 100vw;">
      <q-card-section class="row items-center q-pa-sm q-mb-sm">
        <div class="q-ml-sm-sm">افزودن پیشنهاد محصول</div>
        <q-space/>
      ...
      <q-card-section class="items-center">
        <q-form class="row"
          ...
          <q-card-actions class="bg-white text-teal">
            ...
            <q-btn flat :label="type === 'edit' ?  'ویرایش پیشنهاد محصول': 'ثبت پیشنهاد محصول'" type="submit"/>
          </q-card-actions>
...
export default {
  ...
  methods: {
    onSubmit () {
      ...
      if (this.type === 'create') {
        this.CreateSuggest(app, formData)
      } else {
        this.UpdateSuggest(app, formData)
      }
    },

    CreateSuggest (app, formData) {
      this.$axios.post(process.env.api + 'api/admin/products/suggests/', formData)
        .then(function () {
          app.showNotif('پیشنهاد محصول مورد نظر با موفقیت ثبت شد', 'mdi-checkbox-marked-circle-outline ', 'green')
          app.$parent.$parent.GetAllSuggests()
    ...
    UpdateSuggest (app, formData) {
      this.$axios.put(process.env.api + 'api/admin/products/suggests/' + app.form.id + '/', formData)
        .then(function () {
          app.showNotif('پیشنهاد محصول مورد نظر با موفقیت ویرایش شد', 'mdi-checkbox-marked-circle-outline ', 'green')
          app.$parent.$parent.GetAllSuggests()
          app.$parent.$parent.GetRequiredList()
          app._dialog = false
        })
