  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="ActiveSliders_list" icon="mdi-format-list-text" label="اسلایدر های فعال"/>
          <q-tab class="text-primary" name="InActiveSliders_list" icon="mdi-format-list-text"
                 label="اسلایدر های غیر فعال"/>
        </q-tabs>
        <q-separator/>

        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="ActiveSliders_list">
                ...
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      ...
                      |
                      <q-btn @click="DeleteSlider(props.row.title , props.row.id)" style="color: red" rounded
          ...
          <q-tab-panel name="InActiveSliders_list">
                      <q-input style="width: 300px; float: left !important;"
                     dense v-model.lazy="search"
                     debounce="250"
                     filled type="search">
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      <q-btn
                        @click="EditSlider(props.row.id , props.row.user , props.row.category , props.row.status)"
                        style="color: #4facfe" rounded
                        icon="mdi-square-edit-outline" label="ویرایش" size="md" flat
                        dense>
                        <q-tooltip>ویرایش</q-tooltip>
        ...
        <Modal :showDialog.sync="showDialog" :form="form" :type="type" :request_type="request_type"></Modal>
      </q-card>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    SetStatusFalse (title, id) {
      ...
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید وضعیت اسلایدر ' + ' ( ' + title + ' ) ' + 'نقض شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.post(process.env.api + 'api/admin/sliders/change/status/', formData)
          .then(function () {
            if (app.paginationPage === app.lastPage && app.data.length === 1) {
              app.paginationPage -= 1
            } else {
              app.GetAllActiveSliders()
            }
            app.showNotif('اسلایدر مورد نظر با موفقیت غیر فعال شد', 'mdi-checkbox-marked-circle-outline', 'green')
          })
      })
    },
        ...
        app.$axios.post(process.env.api + 'api/admin/sliders/change/status/', formData)
          .then(function () {
            if (app.paginationPage === app.lastPage && app.data.length === 1) {
              ...
              app.GetAllDisableSliders()
            }
            app.showNotif('اسلایدر مورد نظر با موفقیت فعال شد', 'mdi-checkbox-marked-circle-outline', 'green')
          })
      })
    },
    ...
    DeleteSlider (name, id) {
      const app = this
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید اسلایدر ' + ' ( ' + name + ' ) ' + 'حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        if (app.request_type === 'active') {
          app.$axios.delete(process.env.api + 'api/admin/sliders/active/' + id)
            .then(function () {
              app.showNotif('اسلایدر مورد نظر با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
          ...
          app.$axios.delete(process.env.api + 'api/admin/sliders/disable/' + id)
            .then(function () {
              app.showNotif('اسلایدر مورد نظر با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
    ...
    EditSlider (id) {
      ...
      if (this.request_type === 'active') {
        this.$axios.get(process.env.api + 'api/admin/sliders/active/' + id + '/')
        ...
        this.$axios.get(process.env.api + 'api/admin/sliders/disable/' + id + '/')
          .then(function (response) {
            app.form = response.data
            app.setDialog('edit')
    ...
    GetAllActiveSliders () {
      ...
      this.$axios.get(process.env.api + 'api/admin/sliders/active/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
    ...
    GetAllDisableSliders () {
      const app = this

      this.$q.loading.show()

      this.$axios.get(process.env.api + 'api/admin/sliders/disable/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
          app.data = response.data.results
          app.lastPage = response.data.last_page
  ...
  watch: {
    search () {
      ...
      if (this.tab === 'ActiveSliders_list') {
        this.GetAllActiveSliders()
      } else {
        this.GetAllDisableSliders()
      }
    },
    paginationPage () {
      if (this.paginationPage === 0) {
    ...
    tab (value) {
      ...
      if (value === 'ActiveSliders_list') {
        this.request_type = 'active'
        this.GetAllActiveSliders()
      } else {
        this.request_type = 'disable'
        this.GetAllDisableSliders()
      }
    }
  },
