  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="role_user_list" icon="mdi-format-list-text" label="نقش-کاربری"/>
        </q-tabs>
        <q-separator/>

        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="role_user_list">
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      <q-btn @click="EditRoleUser(props.row.id , props.row.role , props.row.user)"
                      ...
                      |
                      <q-btn
                        @click="DeleteRoleUser(props.row.role.title + '-' + props.row.user.username , props.row.id)"
...
export default {
  ...
  methods: {
    ...
    DeleteRoleUser (name, id) {
      const app = this
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید نقش-کاربری ' + ' ( ' + name + ' ) ' + 'حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.delete(process.env.api + 'api/admin/acl/role_user/' + id)
          .then(function () {
            app.showNotif('نقش-کاربری مورد نظر با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
            app.GetAllRoleUsers()
            app.GetUsersListAndRolesList()
          })
          .catch(function () {
    ...
    EditRoleUser (id, role, user) {
      const app = this

      this.$axios.get(process.env.api + 'api/admin/acl/role_user/' + id + '/')
        .then(function (response) {
    ...
    GetAllRoleUsers () {
      ...
      this.$axios.get(process.env.api + 'api/admin/acl/role_user/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
