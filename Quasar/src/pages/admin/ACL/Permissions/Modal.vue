  ...
  <q-dialog v-model="_dialog">
    <q-card style="width: 750px; max-width: 100vw;">
      <q-card-section class="row items-center q-pa-sm q-mb-sm">
        <div class="q-ml-sm-sm">افزودن دسترسی</div>
        <q-space/>
...
export default {
  ...
  methods: {
    onSubmit () {
      ...
      if (this.type === 'create') {
        this.CreatePermissions(app, formData)
      } else {
        this.UpdatePermissions(app, formData)
      }
    },

    CreatePermissions (app, formData) {
      this.$axios.post(process.env.api + 'api/admin/acl/permissions/', formData)
        .then(function () {
          app.showNotif('دسترسی مورد نظر با موفقیت ثبت شد', 'mdi-checkbox-marked-circle-outline ', 'green')
          app._dialog = false
        })
        .finally(function () {
          app.$q.loading.hide()
          app.$parent.$parent.GetAllPermissions()
        })
    ...
    UpdatePermissions (app, formData) {
      this.$axios.put(process.env.api + 'api/admin/acl/permissions/' + app.form.id + '/', formData)
        .then(function () {
          app.showNotif('دسترسی مورد نظر با موفقیت ویرایش شد', 'mdi-checkbox-marked-circle-outline ', 'green')
          app._dialog = false
        })
        .finally(function () {
          app.$q.loading.hide()
