  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="permissions_list" icon="mdi-format-list-text" label="دسترسی ها"/>
        </q-tabs>
        <q-separator/>

        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="permissions_list">
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      <q-btn @click="EditPermissions(props.row.id)" style="color: #4facfe" rounded
                             icon="mdi-square-edit-outline" label="ویرایش" size="md" flat
                             dense>
                        <q-tooltip>ویرایش</q-tooltip>
                      </q-btn>
                      |
                      <q-btn @click="DeletePermissons(props.row.title , props.row.id)" style="color: red" rounded
...
export default {
  ...
  methods: {
    ...
    DeletePermissons (name, id) {
      const app = this
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید دسترسی ' + ' ( ' + name + ' ) ' + 'حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.delete(process.env.api + 'api/admin/acl/permissions/' + id)
          .then(function () {
            app.showNotif('دسترسی مورد نظر با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
    ...
    EditPermissions (id) {
      ...
      this.$q.loading.show()
      this.$axios.get(process.env.api + 'api/admin/acl/permissions/' + id + '/')
        .then(function (response) {
    ...
    GetAllPermissions () {
      ...
      this.$axios.get(process.env.api + 'api/admin/acl/permissions/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
