  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="stars_list" icon="mdi-format-list-text" label="امتیازات"/>
        </q-tabs>
        <q-separator/>
          ...
          <q-tab-panel name="stars_list">
            ...
            <div class="q-pa-md">
              ...
              >
                ...
                <!--                  <q-input dense v-model="type" filled type="search">-->
                <!--                    <template v-slot:append>-->
                <!--                      <q-icon name="search"/>-->
                <!--                    </template>-->
                <!--                  </q-input>-->
                ...
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="score">{{ props.row.score }}</q-td>
                    <q-td key="product">{{ props.row.product.title }}</q-td>
                    <q-td key="user">{{ props.row.user.username }}</q-td>
                    <q-td key="setting">
                      <q-btn @click="EditStar(props.row.id)" style="color: #4facfe" rounded
                             icon="mdi-square-edit-outline" label="ویرایش" size="md" flat
                             dense>
                        <q-tooltip>ویرایش</q-tooltip>
                      </q-btn>
                      |
                      <q-btn @click="DeleteStar(props.row.product.title + '-' + props.row.user.username , props.row.id)"
...
export default {
  ...
  methods: {
    ...
    DeleteStar (name, id) {
      ...
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید امتیاز ' + ' ( ' + name + ' ) ' + 'حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.delete(process.env.api + 'api/admin/stars/' + id)
          .then(function () {
    ...
    EditStar (id) {
      ...
      this.$q.loading.show()
      this.$axios.get(process.env.api + 'api/admin/stars/' + id + '/')
        .then(function (response) {
    ...
    GetAllStars () {
      ...
      this.$axios.get(process.env.api + 'api/admin/stars/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
